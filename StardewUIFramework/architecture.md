...
| v1.1 — HUD widgets, toasts, player‑owned layout (§16.2) | Done | `v2/hud` | `IUIHud` (`CreateHud` / `GetHud` / `DestroyHud`): a hidden `UIMenu` laid out and drawn by `Hosting/HudService` from `Display.RenderedHud`, input through the SMAPI input events when `Interactive`. Toasts (`ShowToast*`, `Hosting/ToastLayer`) drawn from `RenderedHud`, or `RenderedActiveMenu` while a menu is up. `PlayerLayout` on menus (drag by the title strip, collapse button, bottom‑right resize grip; `Hosting/PlayerLayoutController`), `Hosting/WindowLayoutStore` persisted in save data under `"layout"`, `ResetPlayerLayout`, console commands `ui_toast` / `ui_layout_reset`. |
