...
namespace ProfitCalculator.main.ui
{
    ...
    public class DropdownOption : BaseOption
    {
        ...
        {
            get => Container.Instance.Resolve<DropdownOption>(ModEntry.UniqueID);
            set
            {
        ...
        /// <summary> Draws the closed dropdown: the box with the selected choice and the arrow button. </summary>
        private void DrawSelectedChoice(SpriteBatch b)
        {
        ...
        /// <summary> Draws the dropped list: up to <see cref="MaxValuesAtOnce"/> choices from <see cref="ActivePosition"/>, the active one highlighted. </summary>
        private void DrawChoices(SpriteBatch b)
        {
        ...
        public override void ReceiveLeftClick(int x, int y, Action stopSpread)
        {
            ...
            {
                // the click was outside a closed dropdown: nothing to do
            }
        }
    }
}